...
import type { BatCard, GameConfig, GameState, Player, ThrallKind } from '../game/types';
...
export class App {
   ...
   * だから直近の出目と、リングごとの濃さを一列に並べる。
   * 自分が立っているリングには印が付く ―― それが今夜賭けている場所。
   */
  private tideGauge(): string {
    const s = this.state;
    ...
    const veins = [1, 2, 3, 4]
      .map((ring) => {
        ...
        const pct = Math.round(tideChance(ring) * 100);
        return `<i class="vein${mine}${hit}" title="リング${ring} ― 月潮が指す確率 ${pct}%${
          ring === myRing ? '（いまここ）' : ''
        }">${pct}</i>`;
      })
      .join('');

    const rolled = last ? `${last.dice[0]}+${last.dice[1]}=${last.sum} → リング${last.ring}` : 'まだ振られていない';
    return `
      <span class="gauge gauge-tide" title="月潮 ― ラウンドの終わりに2d4。指されたリングに立つ者だけが血を吸える。直近: ${rolled}">
        <span class="gauge-icon">${ICON.tide}</span>
        <span class="veins">${veins}</span>
      </span>
    `;
  }
          ...
          p.thralls.length > 0
            ? `<div class="player-thralls">${p.thralls
                .map((k) => {
                  ...
                  return `<span class="thrall-chip" title="${spec.name} — ${spec.text}">${spec.icon}</span>`;
                })
                .join('')}</div>`
            : ''
        }
  ...
  private renderHand(): void {
    ...
    if (paying) {
      const spec = THRALL_SPECS[(this.targeting as { thrall: ThrallKind }).thrall];
      hint.innerHTML = `${spec.icon}<b>${spec.cost}</b>`;
      hint.title = `《${spec.name}》の対価に切る札を選ぶ`;
    } else {
  ...
  private thrallPanel(): HTMLElement | null {
    ...
    for (const kind of THRALL_ORDER) {
      ...
      button.innerHTML = `
        ...
        <span class="thrall-cost">${owned ? '✓' : `${ICON.bat}${spec.cost}`}</span>
      `;
